...
### 단일 테스터 (기본 — 에이전트팀 비활성화 시 폴백)

서브에이전트로 테스터를 spawn합니다 (Agent tool, `subagent_type: "tester"`, `model: "sonnet"`):
...
> **에이전트팀 기능**으로 테스터를 구성합니다. Agent tool 서브에이전트가 아닌, 에이전트팀의 독립 프로세스로 각 테스터를 spawn합니다.

...
### 테스트 팀 (검증 강도 2+ 또는 에이전트팀 기본)

검증 강도 2+ 또는 테스트 도메인이 2개 이상이면 에이전트팀으로 병렬 테스트합니다.

에이전트팀 활성화 조건 (모두 충족):
1. `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1` 설정
2. 검증 강도 2+ 또는 테스트 도메인이 2개 이상 (UI + API, 사용자단 + 관리자단 등)
...
에이전트팀 비활성화 시 또는 테스트 도메인이 1개일 때 서브에이전트로 폴백합니다.

---
name: test
description: "실행 테스트 — verify 통과 후 브라우저 자동화(UI/UX) + API 테스트를 실제 실행하여 동작을 검증합니다. verify PASS 후 자동 트리거."
---
...
테스트 범위가 넓은 경우 (L 규모, 또는 도메인이 2개 이상) 테스터를 분리합니다:
...
**복수 테스터 spawn** (각각 `model: "sonnet"`):
```
테스터 A: 사용자단 UI 플로우 (model: "sonnet")
테스터 B: 관리자단 UI 플로우 (model: "sonnet")
테스터 C: API 통합 테스트 (model: "sonnet")
```

...
- **change**: 탐색 에이전트팀 호출 조건 단순화 — **규모 무관 항상 호출**
  - 기존: clarify 1단계에서 M/L 후보 판단 게이트(키워드/규모/명시) 통과 시에만 팀 호출, S는 메인 직접
  - 변경: 단순 프롬프트만으로는 규모 추정 자체가 불가능(닭-달걀 문제)하므로 **진입 즉시 팀에 위임**, 팀 요약으로 예비 규모 판정
  - plan 1단계도 동일하게 규모 무관 항상 팀 호출 (기존 S 메인 직접 예외 제거)
  - AC-2/AC-7/AC-11/AC-15의 "S 규모 확정 시 메인 직접" 분기 제거 → 메인이 직접 grep/Read 하지 않는 원칙이 전 단계 일관 적용
  - 폴백 테이블 (d)의 "S 규모 예외" 조항 제거
- **affected**: `skills/clarify/SKILL.md`, `skills/plan/SKILL.md`

...
# v3 → v4 마이그레이션 안내 (AC-7/AC-8 연동)
# 마커 부재 + settings.json에 v3 forge-flow 훅 잔재 감지 시 사용자에게 진입점 안내
MIGRATION_MARKER="$HOME/.claude/forge-flow-migrated.v4"
SETTINGS="$HOME/.claude/settings.json"
if [ ! -f "$MIGRATION_MARKER" ] && [ -f "$SETTINGS" ]; then
  if grep -qE 'forge-flow(-hooks)?/(workflow-state|stop-guard|dangerous-cmd-guard)\.sh' "$SETTINGS" 2>/dev/null; then
    echo '{"additionalContext": "[FORGE-FLOW v4] v3 잔재가 감지되었습니다. /forge-flow:clarify 입력 시 마이그레이션이 자동 진행됩니다."}'
    exit 0
  fi
fi

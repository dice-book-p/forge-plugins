...
| **P1b ⏸** | §1.1 implement Agent팀→Workflow (게이트 뒤) — **라이브 spike 게이트(§8)** | 저널링/재개+worktree. 단 §2 선행 필수 | 중 (머지) → §5.1로 방어 |
| P2 | §5.3 decision-coverage 게이트 | 저비용 traceability | 낮음 |
| P2 | §5.4 verify 명령 plan-time 선언 | verify 단순화 | 낮음 |
...
- **미검증(라이브)**: 실제 발동·judge 채점 분포·합성 품질 — 라이브 SKILL 실행 시.

### P1b — implement Agent팀→Workflow ⏸ (라이브 spike 게이트)

**선결 조건**: nested worktree 격리 + **격리 worktree 편집의 메인트리 병합 메커니즘** 라이브 검증 필요. Workflow 스크립트는 git 직접 실행 불가(FS/Node API 없음) → 병합 주체(격리 에이전트의 Bash vs 워크플로 후 메인)가 미확정. git 레벨 nested worktree는 검증됨(생성·편집·등록 OK), changelog 2.1.161서 `isolation:worktree` 편집차단 버그 수정 확인. 하지만 병합 경로는 라이브 2-에이전트 spike로 확인해야 200줄 작성 안전(advisor). **이 spike는 Workflow 도구 실행 = 사용자 opt-in 필요.**

**격리 정당화(정밀)**: source writes는 P0로 비충돌이나 — 같은 wave unit들이 `검증방식`(빌드/테스트) **동시 실행 시 공유 빌드상태**(`target/`·`node_modules/`·`.pyc`) 경쟁. worktree 격리는 *파일 편집*이 아니라 **동시 빌드**를 위한 것. reconciliation의 충돌검사는 P0 덕에 trivial 단언; 진짜 게이트는 "전체 스위트 함께 실행"(비충돌 writes도 의미적 비양립 가능).

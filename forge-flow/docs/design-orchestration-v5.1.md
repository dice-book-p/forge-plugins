...
### 전체 SKILL 관통 e2e ✅ (샌드박스 /tmp/ff-e2e-full, M규모 병렬과제)

7 phase 전부 연결 검증 — 문자열 유틸 3종(capitalize/reverse/truncate)+index:
- **clarify** → design.md (AC-1~4). **review-req**(라이브 wf_8af719fc, 17에이전트/394k) → **REWORK 정확 포착**(truncate 음수 n: AC본문 `slice(0,음수)`≠빈문자열 vs 리스크 빈+'...' 모순) → design 수정. **워크플로가 실제 결함 탐지**.
- **plan** → work units 4개 + wave 분해(W0 너비3 병렬: capitalize/reverse/truncate disjoint, W1 index 의존) + 커버리지 게이트(전 AC 귀속✓, 미매핑 0) + 확정 M → review-plan 조건 미해당 **스킵**.
- **implement**(메인 단일, 병렬은 별도 입증) → 7파일 wave순서. **verify** → `node --test` 3/3 PASS, index 통합 동작. **complete** → 커밋.
- **전 seam 데이터 흐름 정상**: design→review→plan→wave→커버리지→implement→verify→complete.

### 비용 floor 게이트 ✅ (e2e가 드러낸 개선점, 미커밋)

**관측**: 4파일 순수 유틸이 review-req **17에이전트/394k 토큰** 소비 — 동일 결함을 단일 패스로 ~10k에 포착 가능. **과잉 프로비저닝**(규모 게이트는 strength만 줄일 뿐 fan-out 발동은 막지 않음).

**해결 — `lightweight` 복잡도 게이트 (규모와 직교)**:
- **변경**: review-req/review-plan/verify.js에 `A.lightweight` → 관점 1 + completeness critic 생략 + refuter 1 (17→~3 에이전트). 게이트 편향(불확실=결함유지)은 보존.
- **판정**: plan 4-A0 — 순수로직·외부의존/API/DB/UI/보안 없음·테스트 검증가능·표면 작음 **모두** 충족 시 true, 확신 없으면 false(보수적). review-req는 plan 전이라 design 기반 자가판정(review-req SKILL).
- **wiring**: plan 4-A0 판정 → review-req/review-plan/verify SKILL args에 `lightweight` 주입. 기본 false = 기존 풀 검증 동작 보존.

### 남은 일 (선택)

- 라이브 lightweight 효과 실측(17→~3 에이전트 절감 확인). 머지(feature/orchestration-v5.1 → main).

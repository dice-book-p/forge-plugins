...
| **P1** | §1.1 plan 생성 Workflow화 (judge panel) | 저위험·즉시 이득. 대화형 경계 밖 | 낮음 |
| **P1** | §1.1 implement Agent팀→Workflow (게이트 뒤) | 저널링/재개+worktree. 단 §2 선행 필수 | 중 (머지) → §5.1로 방어 |
...
### 다음 — P1: plan 생성 Workflow화 + implement Agent팀→Workflow(wave 소비)

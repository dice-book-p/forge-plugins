...
**미검증(라이브)**: 실제 Workflow 발동·동시 worktree-add 경쟁·에이전트의 git 절차 준수·reconciliation 병합 품질 — 라이브 e2e 필요(Workflow opt-in). git-레벨 병합 메커니즘은 입증됨.
...
- **라이브 e2e** (브랜치 설치 + 실작업 1건 관통, Workflow opt-in): plan-judge·implement 실발동, 동시 git, 에이전트 절차 준수 검증.
- **잔여 보조**(선택): §5.1 4단계 reconciliation 정식화, §5.2 격리컨텍스트(implement.js에 일부 반영됨), §5.5 TDD-in-plan 확장.

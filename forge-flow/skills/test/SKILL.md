...
PASS 판정 후, **새로운 테스터 1명**을 spawn하여 확인 라운드를 수행합니다. 이전 테스터와 다른 독립 프로세스가 동일 AC를 재테스트합니다.

**수렴 검증 흐름**:
1. 테스트 팀 PASS 판정 후, 새 테스터 1명 spawn (기존 팀과 별도)
2. 새 테스터가 동일 AC 범위를 독립 테스트
3. **0건** → 수렴 확정 → 최종 PASS
4. **이슈 발견** → 수정 → verify부터 재실행
...
2. **rework-log 기록** (verify SKILL.md의 "Rework Log 관리" 규칙에 따름, 차원 태그 `[코드]` 기본 부여)

...
에이전트팀 비활성화 시(`CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS` 미설정)에만 서브에이전트로 폴백합니다.

> 도메인 1개인 경우에도 에이전트팀이 활성화되어 있으면 **에이전트팀 팀원 1명**으로 구성합니다. 서브에이전트 폴백은 에이전트팀이 비활성화된 경우에만 해당합니다.

...
테스트 도메인이 2개 이상이면 에이전트팀으로 병렬 테스트합니다. **에이전트팀이 기본**입니다.

에이전트팀 사용 조건:
1. `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1` 설정 (setup-workflow에서 기본 활성화)
2. 테스트 도메인이 2개 이상 (UI + API, 사용자단 + 관리자단 등) 또는 검증 강도 2+
...
에이전트팀 비활성화 시 또는 테스트 도메인이 1개일 때(팀 구성이 과한 경우) 서브에이전트로 폴백합니다.

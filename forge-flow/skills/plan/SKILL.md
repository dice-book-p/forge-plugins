...
|----|------|---------|----------|---------|---------|------|
| WU-1 | <unit 제목> | AC-1 | <파일/심볼> | 단위테스트 / 수동 / 스킵(사유) | <PASS 판단 명령 또는 조건 — 예: `./gradlew test FooTest`, "화면에 X가 표시됨"> | (없음) 또는 [WU-id, ...] |

> **work unit = 구현 중 단위검증 게이트의 단위.** 자세한 분해·자동 제안·승인 절차는 본 SKILL의 **3-B단계** 참조. implement는 다음 unit 착수 직전 직전 unit의 검증방식을 실행하여 PASS면 진행, FAIL이면 의존 후속 unit 차단(독립 unit은 계속).
...
2. **검증방식 + 검증 기준 자동 제안**: 각 unit에 대해 plan은 아래 우선순위로 검증방식을 자동 제안하고, **검증 기준(PASS 판단 조건)을 구체적으로 함께 명시**한다.
   - CLAUDE.md `## 빌드 명령` 섹션이 미설정이면 → `스킵(빌드 명령 미설정)` / 검증 기준: `—`
   - 변경 대상이 자동화 단위테스트로 검증 가능한 코드(서비스/유틸 등)이면 → `단위테스트` / 검증 기준: 실행 명령 + 기대 결과 (예: `./gradlew test FooTest` PASS)
...
3. **의존 그래프 자동 도출**: 1단계 탐색 에이전트팀의 출력(`[영향 범위]` 호출 체인, `[기존 패턴 요약]`)을 근거로 plan이 unit 간 의존을 추론한다. unit A의 변경 대상이 unit B의 변경 결과를 입력으로 사용하면 A는 B에 의존한다.
   - **순환 참조 탐지**: 의존 그래프에 순환이 발견되면 plan은 즉시 사용자에게 보고하고 unit 재분해를 제안한다(승인 단계로 진행하지 않음).

4. **사용자 일괄 승인** (`AskUserQuestion`):
...
5. **승인된 결과를 design에 기록**: design `## 구현 계획` 하위에 `### work units` 표를 작성한다(clarify의 design 포맷 참조).
...
| id | 제목 | 대상 AC | 변경 대상 | 검증방식 (자동 제안) | 검증 기준 | 의존 |
|----|------|---------|----------|------------------|---------|------|
| WU-1 | ... | AC-1 | ... | 수동 (마크다운 편집) | "목록에 항목 표시됨" | (없음) |
| WU-2 | ... | AC-2 | ... | 단위테스트 (서비스 로직) | `./gradlew test FooTest` PASS | [WU-1] |

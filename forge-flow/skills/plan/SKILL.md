...
|----|------|---------|----------|---------|------|
| WU-1 | <unit 제목> | AC-1 | <파일/심볼> | 단위테스트 / 수동 / 스킵(사유) | (없음) 또는 [WU-id, ...] |
...
2. **검증방식 자동 제안**: 각 unit에 대해 plan은 아래 우선순위로 검증방식을 자동 제안한다(근거 한 줄 명시).
   - CLAUDE.md `## 빌드 명령` 섹션이 미설정이면 → `스킵(빌드 명령 미설정)`
   - 변경 대상이 자동화 단위테스트로 검증 가능한 코드(서비스/유틸 등)이면 → `단위테스트`
   - 변경 대상이 마크다운/JSON/설정 파일이거나 UI/통합이 필요하면 → `수동`
   - 그 외 → `스킵(사유)` 사유 명시
...
| id | 제목 | 대상 AC | 변경 대상 | 검증방식 (자동 제안) | 의존 |
|----|------|---------|----------|------------------|------|
| WU-1 | ... | AC-1 | ... | 수동 (마크다운 편집) | (없음) |
| WU-2 | ... | AC-2 | ... | 단위테스트 (서비스 로직) | [WU-1] |

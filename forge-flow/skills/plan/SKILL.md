...
1. **rework-log 참조**: `.forge-flow/rework-log.md` 존재 시 스캔하여 이번 작업 영향 범위와 관련된 과거 REWORK 패턴 확인. 관련 패턴 발견 시 구현 계획의 주의사항 또는 리스크 항목에 반영
2. **코드 탐색**:
...
### 자가 체크포인트 (L규모만)
구현 진행률 50% 시점에서 자가 점검:
- [ ] 지금까지 구현한 코드가 '따를 기존 패턴'을 준수하는가?
- [ ] 완료된 AC는 실제로 충족되는가?
- [ ] 이슈 발견 시 즉시 수정 (verify까지 미루지 않음)
```

> `자가 체크포인트` 섹션은 **L규모에서만** design 문서에 포함합니다. S/M규모에서는 생략.
...
```
1. S 규모 → 단일 세션 구현 (에이전트팀 스킵)
2. 병렬 가능성 없음 (독립 작업 단위 1개) → 단일 세션 구현
3. CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1 설정됨?
   ├─ YES → 에이전트팀 구성 (기본)
   └─ NO  → 단일 세션 구현
```

> **에이전트팀이 기본입니다.** `setup-workflow`에서 기본 활성화되므로, M/L 규모 + 병렬 가능 시 항상 에이전트팀을 사용합니다.
...
> **에이전트팀으로 팀원을 생성합니다.** 각 팀원은 독립 Claude Code 프로세스로 실행됩니다.
...
3. **에이전트팀 팀원 spawn**: 각 역할별로 팀원을 spawn하되, 아래 프롬프트 템플릿에 따라 컨텍스트 전달

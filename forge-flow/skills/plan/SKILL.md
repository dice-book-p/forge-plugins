...
2. `Agent` 도구 + `team_name` + `name: "중간검수자"` (model 생략 → 세션 모델 상속)
3. 검수자에게 현재까지의 `git diff`, design 문서, '따를 기존 패턴' 테이블 전달
4. 결과 수신 → `TeamDelete`
...
> `중간 검수` 섹션은 **L규모에서만** design 문서에 포함합니다. S/M규모에서는 생략.
...
> 예: S규모 → 표준(1) (Recommended) / 강화(2+) — 2개만 표시.

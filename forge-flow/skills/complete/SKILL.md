...
> **chunk 모드** (상태 파일 `chunk_mode: true`): 각 chunk가 verify PASS 시점에 이미 커밋되어 있습니다. 이 단계는 **잔여 미커밋 변경**(통합 verify REWORK 수정분 등)에 대해서만 수행하고, 없으면 "커밋할 잔여 변경 없음 — chunk 커밋 {N}건 완료"로 보고 후 3단계로 진행합니다.

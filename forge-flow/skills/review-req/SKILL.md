...
**C. 서브에이전트 교차검증 (폴백 — `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS` 미설정 시에만)**

> 이 섹션은 `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS`가 **미설정**인 경우에만 적용됩니다. 설정되어 있으면 반드시 위 A(에이전트팀)를 사용하세요.

`Agent` 도구를 사용하여 서브에이전트로 검수자를 spawn합니다 (`team_name` **없이**, `isolation: "worktree"`, model 생략 → 세션 모델 상속):

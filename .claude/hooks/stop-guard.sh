...
INPUT=$(cat)
SESSION_ID="c53198d8-ec51-4626-a452-415ba1198d88"
...
if command -v jq >/dev/null 2>&1; then
  _json_read() { jq -r "" "" 2>/dev/null; }
  _json_read_stdin() { printf '%s' "" | jq -r "" 2>/dev/null; }
  _json_set_int() { jq --argjson v "" ". = \$v" "" > ".tmp" && mv ".tmp" ""; }
else
  _json_read() { python3 -c "import json; print(json.load(open('')).get('${1#.}',''))" 2>/dev/null; }
  _json_read_stdin() { python3 -c "import json; print(json.loads('''''').get('${2#.}',''))" 2>/dev/null; }
  _json_set_int() { python3 -c "
import json
with open('') as f: d=json.load(f)
d['']=
with open('','w') as f: json.dump(d,f,ensure_ascii=False)
" 2>/dev/null; }
fi

...
> 상태: 설계 확정 (v2.3 스냅샷)
> 스냅샷 일시: 2026-03-13
> 변경: UserPromptSubmit 역할 재정의, stop_hook_active stdin 활용, 체이닝 2중 구조, 에이전트팀 병렬 기준, MCP 비종속 전면 반영
...
#### Layer 2: UserPromptSubmit 훅 — 워크플로 진입 보장 + 상태 알림

사용자가 프롬프트를 입력할 때만 발화. 역할:
- **초기 진입**: 상태 파일 없으면 "clarify 먼저 실행" 주입 → 워크플로 시작 강제
- **세션 복구**: orphan design 파일 감지 → 이전 작업 이어갈지 확인
- **상태 알림**: 구현 중이면 "verify 필수" 리마인드
- 스킬 간 자동 체이닝에는 관여하지 않음 (자동 진행 중 미발화)
...
```bash
#!/bin/bash
INPUT=$(cat)  # stdin에서 stop_hook_active, last_assistant_message 수신
SESSION_ID="${CLAUDE_SESSION_ID}"
STATE_FILE=".forge-flow/state-${SESSION_ID}.json"

# 1. 상태 파일 없으면 → 워크플로 밖, 통과
if [ ! -f "$STATE_FILE" ]; then
  exit 0  # JSON 미출력 = 정상 종료 허용
fi
...
# 3. Circuit breaker — stdin의 stop_hook_active 활용
...
if [ "$STOP_HOOK_ACTIVE" = "true" ] && [ "$STOP_COUNT" -ge 3 ]; then
  # 이미 stop hook으로 재시작된 상태 + 3회 연속 → 강제 통과
...
| 스킬 간 자동 체이닝 | 프로그래밍적 호출 불가 | (1) SKILL.md 마지막에 "완료 후 다음 스킬 실행" 지시 포함, (2) description 매칭. 자동 진행 중에는 이 2개로 체이닝. UserPromptSubmit은 초기 진입 보장 + 상태 알림 역할 (아래 참고) |

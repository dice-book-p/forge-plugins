...
| 에이전트팀 미활용 | L 규모에서 에이전트팀 활용 | 병렬 구현 + 대화형 리뷰 |
...
**실행 조건**: L 필수. M은 새 패턴 도입 또는 모듈 간 영향 시만.
...
- **활성화 시**: settings.local.json에 환경변수 주입, L 규모에서 에이전트팀 제안
- **비활성화 시 (기본값)**: 환경변수 미주입, L 규모도 M 워크플로로 폴백
...
| 에이전트팀 | M 워크플로로 폴백 (서브에이전트 교차검증) |
| 외부 LLM | 서브에이전트 교차검증 (동일 모델이지만 독립 컨텍스트) |
...
| **Hook (Stop, agent 타입)** | 검수 완료 전 종료 차단 | ok: false 시 Claude 계속 작업 |
| **Hook (PreToolUse)** | 위험 작업 차단 | DB 변경, force push 등 |
| **서브에이전트 (Agent tool)** | 교차검증 수행 | worktree 격리로 독립 판단 |
| **에이전트팀** | L 규모 병렬 구현 + 대화형 리뷰 | 자연어로 생성, 사용자 승인 |
...
if [ -f "$STATE_FILE" ]; then
  ...
  # 새 세션이지만 design/ 파일 존재 — orphan 감지 (다른 세션에서 참조 중인 파일 제외)
  ORPHANS=""
...
| `stop_count` | stop-guard.sh (Stop 훅) | Stop 이벤트 발생 시 +1, 스킬이 phase 전이 시 0으로 리셋 |
...
2. **새 세션에서 복구** (세션 종료 후 재시작):
   ...
   - **design/ 파일 기반 복구**: UserPromptSubmit 훅이 design/ 디렉토리를 스캔
     ...
     - 구현 계획 있음 + verify 기록 없음 → `implementing`부터 재개
     - verify FAIL 이력 있음 → 수정 후 `verify` 재실행
   - 새 상태 파일을 현재 phase로 생성하여 루프 재개
...
2. **Phase 2**: 워크플로 강제 + 자동 실행
   - Stop 훅 (agent 타입) 구현
   - UserPromptSubmit 훅 — 상태 파일 기반 동적 컨텍스트 주입
   - PreToolUse 훅 — 위험 작업 차단
   - CLAUDE.md 템플릿 업데이트

3. **Phase 3**: 교차검증 + 에이전트팀
   - verify에 규모별 검수 로직 (S: 경량, M: 서브에이전트, L: 3중 검수)
   - review-req에 서브에이전트 요구사항 검수
   - 에이전트팀 역할 템플릿 + 제안 로직

4. **Phase 4**: 세션 독립 루프 + 복구
   - `.forge-flow/state-{SESSION_ID}.json` 상태 관리
   - Stop 훅 기반 루프 + circuit breaker
   - 세션 복구 메커니즘 (기존 아티팩트 감지)
